...
def mmhdome_or_basin(img, value, dome = True):
...
def find_and_highlight_craters(image, kernel):
    ...
    cv2.imwrite('mmadmsolo2.jpg', img)
    img = mmhdome_or_basin(img, 10, True)
    cv2.imwrite('mmhdomesolo2.jpg', img)
    img = mmhdome_or_basin(img, 20, False)
    cv2.imwrite('mmhbasinsolo2.jpg', img)
    img = morphological_gradient(img, kernel)
    cv2.imwrite('gradientsolo2.jpg', img)
    img = mmbinary(img, 32)
    cv2.imwrite('binarysolo2.jpg', img)
    img = image_closing(img, kernel)
    cv2.imwrite('closingsolo2.jpg', img)
    img = image_opening(img, kernel)
    cv2.imwrite('openingsolo2.jpg', img)
    find_draw_and_measure_contours(resize, img)
    cv2.imwrite('finalsolo2.jpg', resize)


kernel = create_kernel(9, 1) #cria o kernel com o tamanho NxN e raio especificados
find_and_highlight_craters('Solo_2.jpg', kernel)